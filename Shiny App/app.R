...
movie_predict <- glm(log(gross_adj) ~ budget +  rating + season + widest_opening, data=movies)
...
ui <- pageWithSidebar(
  ...
  sidebarPanel(
    ...
    p('Predicted Domestic Earnings: $178,895,704'),
    p('Actual Domestic Earnings: $174,355,724'),
    
    
    strong("The Help (2011)"),
    p('Budget: $25 million, Widest Opening: 3,014, Rating: PG-13, Season: Summer'),
    p('Predicted Domestic Earinings: $156,818,192'),
    p('Actual Domestic Earnings: $192,820,944'),
    
    strong("Dunkirk (2017)"),
    p('Budget: $100 million, Widest Opening: 4,014, Rating: PG-13, Season: Summer'),
    p('Predicted Domestic Earinings: $213,814,021'),
...
# This function defines server logic to calculate the prediction and create the necessary graphs. 
server <- function(input, output) {
  
  
  
  
  # This takes the values from the sliders and inputs and puts them in the predcitive model.
